...
export const CategoryValue = (data, aidCategoryItem) => {
  return data
    .filter(
      ({ aidCategory }) => aidCategory === convertToString(aidCategoryItem)
    )
...
export const CreateArrayAgencyType = (data) => {
  ...
  data.reduce(function (res, item) {
    ...
    res[item.agency].value += parseInt(item.value);
    return res;
  }, {});
  return newArray;
};
  ...
  data.reduce(function (res, item) {
    if (!res[item.beneficiaryType]) {
      res[item.beneficiaryType] = {
        label: titleCase(item.beneficiaryType),
        value: 0,
      };
